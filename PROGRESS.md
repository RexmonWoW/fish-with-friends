...
- **Stats groundwork done (2026-09-02)**: cast range, reel zone height/shrink/fill rate/QTE window, and bite pacing (hook window/spook pause/call radius) all moved off hardcoded consts into `PlayerStats` (`data/player_stats.gd`), owned by `Player.stats`. No shop yet -- that's next, and it needs a real lobby to live in first.
...
- Not built yet: real lobby content, real multi-player livewell "negotiate when full" UX, real fish species/art content, rarity-tiered spawn weighting (roll a tier first, then weight within it -- separate from the distance-based spatial rarity already built).
