...
8. [~] Shared livewell — data layer real (`add_fish()`/`remove_fish()`/`is_full()`), catches land in a slot, and each filled slot now shows a placeholder capsule (tinted by rarity) parented to that slot's marker — static for now, swimming/animated fish is the planned next pass. Still missing: negotiation/throw-overboard interaction, "any player can look at the livewell and see stats" UI, and multiplayer replication (visuals only spawn host-side right now).
...
## Next Up
- Swimming livewell fish — currently a static placeholder capsule per slot (see below). Next pass: replace with per-slot wander/bob motion.
