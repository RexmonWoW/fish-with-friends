...
11. [~] Walkable lobby — bare-bones room + start trigger + Per-Run Shop counter. Mirror/trophy case still not started (cosmetics don't exist yet).
...
- **Casting rework, all 3 steps done (2026-09-01)**: landing preview (green/red), arc first-hit collision, dead-cast-lies-there (no bounce), and bonk are all built and headless-tested. Still needs real 2-machine confirmation, especially the bonk (client-authoritative knockback broadcast) and the WaterValidator player-exclusion caveat.
...
- **Stats groundwork done (2026-09-02)**: cast range, reel zone height/shrink/fill rate/QTE window, and bite pacing (hook window/spook pause/call radius) all moved off hardcoded consts into `PlayerStats` (`data/player_stats.gd`), owned by `Player.stats`.
- **Per-Run Shop v1 done (2026-09-02)**: counter in the lobby, host-authoritative purchases (`ShopCounter`), item list in `ShopCatalog`. Only the counter + interaction were built, not the rest of the lobby (mirror/trophy case) -- still not started, cosmetics don't exist yet. Playtest fix: the UI never actually appeared (root Control hidden in `_ready()`, nothing ever re-showed it) -- fixed.
...
- Balance placeholder numbers: quota formula constants, Big Fish Event tunables, QTE tunables, livewell swim/look-angle tunables, ambient shadow population (count/spread/swim speed/call-radius growth rate/max call radius/spatial rarity bias strength), capsize toss impulse strength/settle time, reel fill-rate distance falloff, reel bar's max stretch distance, `ShopCatalog` prices (cast range tiers 60/90/130, line strength 50, reel upgrade 50, bait 20, fish finder 150 -- picked so day 1's surplus buys roughly one thing, not tested against real playtest numbers yet), bait/fish-finder's 1.5x/1.4x bite-radius multipliers (`BiteEventManager`), and rod smack's range/cone/impulse/cooldown (`Rod` -- 2.5m, 50°, 100/80, 1s).
...
- Not built yet: lobby mirror/trophy case (needs cosmetics first), real multi-player livewell "negotiate when full" UX, real fish species/art content, rarity-tiered spawn weighting (roll a tier first, then weight within it -- separate from the distance-based spatial rarity already built).
...
- **Remove the debug console** (`skip_round`/`skip_day`/`set_time`/`add_money`, added pass 29, `add_money` added 2026-09-02) and any other dev cheats before release.
...
- 2026-09-01 (spatial rarity) — `VisualFishSpawner` now rolls a shadow's species AFTER its home point, biasing `SpawnPool.roll_species` toward the lake pool's own above/below-average `base_value` species by distance from the boat -- purely off each species' existing `spawn_weight`/`base_value`, nothing hardcoded, so new fish slot in automatically.
- 2026-09-01 (casting rework steps 2-3) — Preview valid color fixed to green (blue-on-blue was unreadable). `_validate_and_land_cast` now sweeps the whole flight path (stepped raycasts along the same parabola `Bobber.position_at` draws) instead of only probing the endpoint; first hit wins. Non-water = dead cast, plops and lies there, no bounce physics (the parked attempt's approach, deliberately not repeated). Hitting a player = bonk (light knockback broadcast, self-applied on the victim's own client, same pattern as the capsize toss) plus a dead cast at their feet. The sweep does its own raw raycasts rather than reusing `WaterValidator.find_water_point` -- that helper deliberately re-casts past Player bodies for the water-height snap, which would make a bonk impossible to ever detect. Preview reuses the same sweep, so it reads blocked exactly when the real cast would be. Closes the old "rejected-cast feedback" item -- no more silent `invalid_water` rejections.
- 2026-09-02 (stats refactor) — Pulled every upgrade-sellable number (cast range, reel zone height/shrink-per-miss/fill rate/QTE window, bite hook window/spook pause/call radius) out of hardcoded consts on Rod/ReelMinigame/BiteEventManager into a new `PlayerStats` resource owned by `Player.stats` -- plain, serializable, no node refs, exactly what a run save will persist per Steam ID later. Groundwork only, no shop yet. Default cast range cut to 40% of the old max, deliberate per direction -- paired with spatial rarity, range is now the main progression axis. Confirmed and closed two Open Questions: quota-paid-from-pot stays as built, and the blind-cast search-radius pacing is fine as tuned.
- 2026-09-02 (Per-Run Shop v1) — Built the shop counter (lobby only, `ShopCounter`) and its host-authoritative purchase flow: clients request, the host validates against `ShopCatalog` + `RunState.try_spend_from_pot`, applies the effect, broadcasts the result to everyone (a bystander sees a crewmate's purchase land live, not just the buyer). Cast range (3 tiers), line strength, reel upgrade, bait (consumable, stacks, ticks down at round end) are per-player, written straight into `PlayerStats`; fish finder is crew-wide (`RunState.has_fish_finder`) and stacks multiplicatively with bait on the bite call-radius. New `NetworkManager.broadcast_player_stats` syncs a purchased player's stats to every peer -- the first real use of the sync path the stats refactor left unbuilt. Added an `add_money` debug console command for testing the shop without grinding catches.
- 2026-09-02 (playtest fixes) — Shop UI never appeared: `ShopDisplay._ready()` hid the root Control and nothing ever showed it again, so both the hint and the panel stayed invisible regardless of their own state -- dropped the root `hide()` (grepped every other overlay for the same shape, only this one had it). Line tangling now actually stops both players' fishing dead, not just the reel-popup overlay it did before: `BiteEventManager` polls a new `TangleManager.is_tangled()` through every phase of the bite sequence (shadow search, travel, hook window, spook pause) and freezes elapsed time on whichever phase is running instead of ticking on underneath the duel; hook-set is also rejected server-side while tangled as a second guard, since mashing and hook-set are both "cast." Added a defensive (currently unreachable) reel-fight cancel if a tangle ever starts while someone's REELING.
- 2026-09-02 (rod smack) — New melee swing on its own key (F): short cone in front of the camera, host detects the hit and broadcasts a light knockback -- reuses the cast bonk's exact `player_bonked` broadcast, no parallel plumbing. No damage, 1s cooldown, works in any rod state including no cast at all.
