...
## Next Up
- **2-machine test in progress (passes 31-32) — movement CONFIRMED working.** Everything else still needs re-verification after two full rounds of real bugs surfacing as soon as things became visible/testable: livewell full-sync (pass 27), sell-every-round (pass 30), Big Fish Event lobby-trigger race/marker/QTE-label/boat-clearance (31-32), capsize toss/tilt/text-position (31-32), round timer sync (31, then re-broken and re-fixed in 32), cast-cancel (32), livewell round-end selling gaps + swap feature (32). Treat nothing here as done until a clean pass with no new reports.
